...
static wxString projection_name[NUM_PROJECTIONS]
    = { _("Mercator"), _("Orthographic"), _("Polar"), _("Stereographic"),
          _("Gnomonic"), _("Equirectangular") };
static OcpnProjTypePI projection_map[NUM_PROJECTIONS]
    = { PI_PROJECTION_MERCATOR, PI_PROJECTION_ORTHOGRAPHIC, PI_PROJECTION_POLAR,
          PI_PROJECTION_STEREOGRAPHIC, PI_PROJECTION_GNOMONIC,
...
int projections_pi::Init(void)
{
    ...
    for (int i = 0; i < NUM_PROJECTIONS; i++) {
        m_projection_menuitem[i] = new wxMenuItem(projection_menu, -1,
            projection_name[i], wxEmptyString, wxITEM_CHECK);
        m_projection_menu_id[i]
            = AddCanvasContextMenuItem(m_projection_menuitem[i], this);
    ...
    return (WANTS_CONFIG | WANTS_ONPAINT_VIEWPORT | WANTS_PREFERENCES);
}
...
int projections_pi::GetAPIVersionMajor() { return MY_API_VERSION_MAJOR; }

int projections_pi::GetAPIVersionMinor() { return MY_API_VERSION_MINOR; }

int projections_pi::GetPlugInVersionMajor() { return PLUGIN_VERSION_MAJOR; }

int projections_pi::GetPlugInVersionMinor() { return PLUGIN_VERSION_MINOR; }

wxBitmap* projections_pi::GetPlugInBitmap() { return _img_projections; }

wxString projections_pi::GetCommonName() { return _("Projections"); }
...
{
    for (int i = 0; i < NUM_PROJECTIONS; i++)
        m_projection_menuitem[i]->Check(
            projection_map[i] == vp.m_projection_type);
}
